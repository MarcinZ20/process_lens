...
class ProcessMiner:
    ...
    def prepare_data(self, case_col: str, activity_col: str, timestamp_col: str):
        ...
        self.log[timestamp_col] = pd.to_datetime(
            self.log[timestamp_col], format="mixed", dayfirst=True, errors="coerce"
        )

        if self.log[timestamp_col].isna().any():
            print(
                f"Warning: Dropped {self.log[timestamp_col].isna().sum()} rows with invalid dates."
            )
    ...
    def mine_and_decompose(
        self, resolution: float = 1.0
    ) -> Tuple[nx.DiGraph, Dict[int, list], Dict[str, int]]:
        ...
        communities = nx.community.greedy_modularity_communities(
            undirected_G, weight="weight", resolution=resolution
        )
