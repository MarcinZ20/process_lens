...
def main():
    ...
    if 'col_case' not in st.session_state: st.session_state.col_case = all_columns[0]
    if 'col_act' not in st.session_state: st.session_state.col_act = all_columns[1] if len(all_columns) > 1 else all_columns[0]
    if 'col_time' not in st.session_state: st.session_state.col_time = all_columns[2] if len(all_columns) > 2 else all_columns[0]
    ...
    with col1: case_col = st.selectbox("Case ID", all_columns, key='col_case')
    with col2: act_col = st.selectbox("Activity", all_columns, key='col_act')
    with col3: time_col = st.selectbox("Timestamp", all_columns, key='col_time')

    st.markdown("---")

    def run_analysis():
        st.session_state.analysis_done = True
        if 'nx_graph' in st.session_state: del st.session_state.nx_graph
        if 'subprocess_map' in st.session_state: del st.session_state.subprocess_map
        if 'subprocess_names' in st.session_state: del st.session_state.subprocess_names
        ...
        try:
            if 'nx_graph' not in st.session_state:
                ...
                with st.spinner("Mining & Decomposing..."):
                    ...
                    nx_graph, subprocess_map, activity_mapping = miner.mine_and_decompose(resolution=granularity)
                    ...
                    if enable_ai and llm.is_active:
                        ...
                        for i, (sub_id, activities) in enumerate(subprocess_map.items()):
            ...
            view_options = ["Show Entire Process"] + [str(name) for name in subprocess_names.values()]
            ...
            if selected_view != "Show Entire Process":
                selected_id = next(
                    (id for id, name in subprocess_names.items() if str(name) == selected_view),
                    None
                )
                ...
                if selected_id is not None:
                    ...
                    st.warning("Could not locate the selected subprocess. Showing full graph.")

            col_viz, col_stats = st.columns([3, 1])

            with col_viz:
                visualizer.render_graph(graph_to_render, activity_mapping, subprocess_names)

            with col_stats:
                st.info(f"**Nodes:** {len(graph_to_render.nodes)}\n\n**Edges:** {len(graph_to_render.edges)}")
                if selected_view != "Show Entire Process":
                    st.caption("You are viewing an isolated subprocess. Select 'Show Entire Process' to see the full flow.")
